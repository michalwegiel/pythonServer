...
from website.routes import actions, routes
...
def page_not_found():
    ...
    return HTTPResponse(
        status_code=HTTPResponseCode.NOT_FOUND, status_info="PAGE NOT FOUND", content_type="text/html", content=content
    )
...
def response_factory_get(route: str):
    ...
    if view is not None:
        ...
        return HTTPResponse(
            status_code=HTTPResponseCode.OK, status_info="OK", content_type=content_type, content=content
        )
...
def response_factory_post(action: str, data: str):
    """
    Returns an HTTP response for a POST request.
    """
    parsed_data = {k.replace("-", "_"): v[0] for k, v in parse_qs(qs=data, keep_blank_values=True).items()}
    post_method = actions.get(action, None)
    if post_method is not None:
        action_result = post_method(**parsed_data)  # type: ignore
